...
\usepackage{epstopdf}
\usepackage{tikz}
\usetikzlibrary{arrows,backgrounds,fit,positioning,calc,shapes,math,arrows.meta}
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% Color scheme
\setbeamercolor*{structure}{fg=white, bg=OSTblackberry}
\setbeamercolor*{palette primary}{use=structure, bg=OSTdarkpurple}
\setbeamercolor*{palette secondary}{use=structure, bg=OSTlightpurple}
\setbeamercolor*{frametitle}{use=structure}
\setbeamercolor*{alerted text}{fg=OSTdarkred}
\setbeamercolor*{example text}{fg=OSTdarkgreen}
...
\setbeamercolor{caption name}{fg=structure.bg}

% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% Blocks
\newcommand{\blockbegin}[1]{
	\ifx\insertblocktitle\empty
		\tcolorbox[colback=white,colframe=#1,coltext=black,coltitle=white]
	\else
		\tcolorbox[colback=white,colframe=#1,coltext=black,coltitle=white,title=\insertblocktitle]
	\fi
}
\defbeamertemplateparent{blocks}[framed]{block begin,block alerted begin,block example begin,%
	block example end,block alerted end,block end}[1][]{[#1]}
\defbeamertemplate{block begin}{framed}[1][]{\blockbegin{structure.bg}}
\defbeamertemplate{block alerted begin}{framed}[1][]{\blockbegin{alerted text.fg}}
\defbeamertemplate{block example begin}{framed}[1][]{\blockbegin{example text.fg}}
\defbeamertemplate{block end}{framed}[1][]{\endtcolorbox}
\defbeamertemplate{block alerted end}{framed}[1][]{\endtcolorbox}
\defbeamertemplate{block example end}{framed}[1][]{\endtcolorbox}
\setbeamertemplate{blocks}[framed]
\setbeamertemplate{theorems}[numbered]

% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% Quotes
\defbeamertemplateparent{quotes}[framed]{quote begin,quote end}{}
\RenewEnviron{quote}[1]{\begin{center}
	\begin{tikzpicture}[remember picture, background rectangle/.style={fill=OSTlightpurple},show background rectangle]
		\node[overlay] (OQ) {\fontfamily{LinuxLibertineT-LF}\fontsize{45pt}{45pt}\selectfont``};
		\node[above right=-2pt and -20pt of OQ.north east,inner xsep=20pt,anchor=north west] (text) {\begin{varwidth}{\linewidth-50.5pt}\usebeamerfont{quote}\BODY\end{varwidth}};
		\node[inner ysep=0pt,right=-20pt of text.south east,anchor=west,yshift=-2pt,overlay] (CQ) {\fontfamily{LinuxLibertineT-LF}\fontsize{45pt}{45pt}\selectfont''};
		\ifx#1\empty\else\node[below=0pt of text.south east, inner xsep=20pt,anchor=north east] (author) {-- #1};\fi
...
\newcommand{\ostheadline}[1]{
	\begin{tikzpicture}
		...
		\ifx \SubtitleInHeader \True
			\node[inner sep=0pt,anchor=south east] at (10mm+\linewidth, 3.2mm) (subsection) {\textbf{\scriptsize\color{OSTblack}\insertsubsection}};
		\fi
	\end{tikzpicture}}
